...
class simpleKNNClassifier(ClassifierAlgorithm):
    ...
    def pred_knn(self, test_data):
        ...
        for i, sample in enumerate(test_data):
            ...
            dist = np.linalg.norm.norm(self.data - sample, axis=1) #S(n) T(n)
...
class Experiment:
    ...
    def ROC(self):
        """
        Creates a ROC curve for the simple KNN classifier
        """
        ...
        for i, classifier in enumerate(self.classifiers):
            #get probs from pred_knn
            pred_probs = classifier.pred_knn(self.data)
            #positive class need to be labeled as 1
            pos_pred_probs = pred_probs[:, 1]

            #get true and false positive rates
            tpr, fpr, _ = self.get_tpr_fpr(self.labels, pos_pred_probs)
    ...
    def get_tpr_fpr(self, true_labels, pred_probs):
        """
        get true and false positive rates
        """
        #sort the prob preds and true labels
        sorted_idx = np.argsort(pred_probs)[::-1]   
        sorted_labs = true_labels[sorted_idx]

        tpr = []
        fpr = []

        #compute the tpr and fpr
        for threshold_idx, threshold in enumerate(sorted_labs):
            #use the threshold to get the preds
            threshold_preds = (pred_probs >= threshold).astype(int)
            #get the true positives
            tp = np.sum(threshold_preds == 1) & (sorted_labs == 1)
            #get the false positives
            fp = np.sum(threshold_preds == 1) & (sorted_labs == 0)
            #get the true negatives
            tn = np.sum(threshold_preds == 0) & (sorted_labs == 0)
            #get the false negatives
            fn = np.sum(threshold_preds == 0) & (sorted_labs == 1)

            tpr.append(tp / (tp + fn))
            fpr.append(fp / (fp + tn))

        return np.array(tpr), np.array(fpr), sorted_idx
...
class DecisionTreeClassifier(ClassifierAlgorithm):
    ...
    def train(self, training_data, training_labels):
        self.root = self.__grow_tree(training_data, training_labels)
    ...
    def make_tree(self, data, labels, current_depth=0):
        ...
        if best_feature is not None:
            ...
            #give the most common label if the best feature is none
    ...
    def predict(self, data):
        preds = [self.predict_sample(sample) for sample in data]
        return np.array(preds)
    
    def _predict(self, val, node = None):
        if node is None:
            node = self.root
        if node.label is not None:
            return node.label
        if val[node.feature_idx] < node.threshold:
            return self._predict(val, node.left)
        else:
            return self._predict(val, node.right)
    ...
    def printTree(self, node)
        ...
        decision = f"[X{node.feature_idx} <= {node.threshold}]"

        #get the left and right nodes
        #left
        left = self.printTree(node.left) if node.left is not None else "[None]"

        #right
        right = self.printTree(node.right) if node.right is not None else "[None]"

        return f"{decision} -> {left}, {right}"
